...
const [user, setUser] = useState(null);
  ...
  const buildFavoriteYearShareCardBlob = async () => {
    ...
    ctx.fillText(`My Top 10 of ${year}`, canvas.width / 2, 178);
    ctx.font = '500 22px Segoe UI';
    ctx.fillStyle = '#93c5fd';
    ctx.fillText('By Flickd Rank � Personal Year Card', canvas.width / 2, 218);
    ...
    ctx.font = '500 20px Segoe UI';
    ctx.fillText('stats generated by', canvas.width / 2 - 46, footerY);

    const logo = await loadImage('/flickd-favicon.png');
    if (logo) {
      ctx.drawImage(logo, canvas.width / 2 + 66, footerY - 22, 26, 26);
    }
